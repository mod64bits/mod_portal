...
class UnifiClient(object):
    ...
    def __init__(self):
        ...
        self.__session.mount(self._get_resource_url(),
                             SSLAdapter(ssl.PROTOCOL_SSLv23))
    ...
    def _get_resource_url(self, path_name=None):
        ...
        if path_name:
            ...
            url = str.format('{0}{1}:{2}/{3}', 'https://',
                             self.__unifiServer, '8443', path_name)
            logger.debug('URL: {0}'.format(url))
            return url
        else:
            url = str.format('{0}{1}:{2}/', 'https://',
                             self.__unifiServer, '8443')
            return url
    ...
    def _is_authorized(self, guest_mac):
        ...
        try:
            client_found = next(
                client for client in client_list if client["mac"] == guest_mac)
    ...
    def login_on_unifi_server(self):
        ...
        login_response = self.__session.post(
            login_url, data=json.dumps(data), verify=False)
